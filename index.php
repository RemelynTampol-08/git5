...
$acc_fullname="Mark Jastine";
$acc_address="Marinduque Ph";

//check_url
$url_add = "http://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'];
//condition to know if the button in clicked

	if(isset($_REQUEST['login_button'])===true){
		//GET THE USER NAME AND PASSWORD

		//kapag mali yong username
		if($_REQUEST['form_username'] != $acc_name){
			header("Location: ".$url_add. "?notexist");
		}
		else if($_REQUEST['form_username'] == $acc_name && $_POST['form_password'] != $acc_password){
			header("Location: ".$url_add. "?wrongpass");
		}else if($_REQUEST['form_username'] == $acc_name && $_POST['form_password'] == $acc_password){
			header("Location: ".$url_add."?sucess");

			//session variable
			$_SESSION['ses_username'] = $acc_name;
				...
				<div class="col-md-6 col-lg-5">
		      	...
		      	<h3 class="text-center mb-4">MyTumblr Login</h3>
		      		...
		      		<div class="form-group">
						...
						} else if(isset($_REQUEST['sucess'])===true){
						...
						echo "<div class ='alert alert-info' role='alert'> Please Login First...</div>";
							...
							echo "<div class ='alert alert-warning' role='alert'> You're still logged in. Plese <a href='account.php'>Click here</a> to proceed.</div>";
						}
						?>
